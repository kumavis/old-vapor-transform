...
function transformJs(src, opts) {
  ...
  transformAstImplicitGlobalDefs(ast, scope)
...
function transformAstImplicitGlobalDefs(ast, scope){
...
function implicitGlobalFnAssignment(identifier){
  return {
    ...
    expression: {
      ...
      operator: '=',
      left: createGlobalMemberExpression(identifier),
      right: identifier,
    },
  }
}
...
function transformImplicitGlobalToExplicitGlobal(node){
  ...
  node.left = createGlobalMemberExpression(identifier)
}
...
function createGlobalMemberExpression(identifier){
  return {
    type: 'MemberExpression',
    computed: false,
    object: {
