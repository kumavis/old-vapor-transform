...
function transformAstForImplicitGlobalDefs(ast, scope){
  ...
  injectLocalGlobalReferences(scope)
}
...
function transformImplicitGlobalToExplicitGlobal(node){
  ...
  node.right = {
    type: 'AssignmentExpression',
    operator: '=',
    left: createGlobalMemberExpression(name),
    right: right,
  }
}
...
// creates a local variable at the top of the scope where an implicit global is set
function injectLocalGlobalReferences(scope) {
  var globalVars = scope.implicit.variables.slice().reverse()
  globalVars.forEach(function(varData){
    var body = varData.scope.block.body
    var node = {
